#[macro_use] extern crate rocket; // because we want to use the rocket annotations
...
struct User {
    id: Option<i32>, // will be automatically assigned by the system
    name: String,
    email: String,
}
...
async fn add_user(
    conn: &State<Client>, // connection
    user: Json<User> // user in json format and uses struct User
) -> Result<Json<Vec<User>>, Custom<String>> { // in case we have an error
    execute_query(
        conn,
...
async fn get_users_from_db(client: &Client) -> Result<Vec<User>, Custom<String>> {
    let users = client
        .query("SELECT id, name, email FROM users", &[]).await // SQL query
...
async fn rocket() -> _ {
    ...
    // create the table if the user doesn't exist
    client
        .execute(
